...
beforeEach(() => {
  ...
  mockPeer = {
    on: vi.fn(),
    connect: vi.fn().mockReturnValueOnce(mockConn).mockReturnValue(mockRetryConn),
    destroy: vi.fn(),
  }
  MockPeer.mockImplementation(function () { return mockPeer })
})
...
function triggerPeer(event: string, ...args: unknown[]) {
  const found = (mockPeer.on.mock.calls as [string, (...a: unknown[]) => void][]).find(([e]) => e === event)
  found?.[1](...args)
}
...
describe('createPeerSession – host role', () => {
  ...
  it('send does nothing when conn is null (before any connection)', () => {
    ...
    expect(() => handle.send({ type: 'sync', state: createClassicGameState() })).not.toThrow()
  })
...
describe('createPeerSession – joiner role', () => {
  ...
  it('does not register connection event handler for joiner', () => {
    ...
    const registeredEvents = (mockPeer.on.mock.calls as [string, unknown][]).map(([e]) => e)
